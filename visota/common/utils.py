...
def upload_category_img_to(instance, filename):
    return 'images/categories/{now}-{filename}'.format(filename=filename, now=int(time() * 1000))

def upload_group_img_to(instance, filename):
    return 'images/groups/{now}-{filename}'.format(filename=filename, now=int(time() * 1000))
...
def generate_unique_slug_translated(klass, klass2, field):
    ...
    unique_slug = origin_slug
    numb = 1
    while klass.objects.filter(translations__slug=unique_slug).exists() or (klass2 is not None and klass2.objects.filter(old_slug=unique_slug).exists()):
        unique_slug = f'{origin_slug}-{numb}'
        numb += 1
    return unique_slug